    ...
    public static class YouthPolicyItem {
        ...
        public YouthPolicy toEntity() {
            return YouthPolicy.builder()
                    ...
                    .policyField(combineFields(this.lclsfNm, this.mclsfNm, " / ")) // 두 개의 분류를 하나로 조합
                    .supportContent(this.plcySprtCn)
                    .operationPeriod(combineFields(this.bizPrdBgngYmd, this.bizPrdEndYmd, " ~ ")) // 시작일과 종료일을 하나로 조합
                    .applicationPeriod(this.aplyYmd)
                    .supportScale(this.sprtSclCnt)
                    .minAge(this.sprtTrgtMinAge)
                    .maxAge(this.sprtTrgtMaxAge)
                    .residence(this.zipCd)
                    .incomeCondition(buildIncomeCondition()) // 여러 소득 정보를 하나의 의미있는 문자열로 조합
